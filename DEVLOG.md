...
此外第 2.7 节"检测报告生成"补充了 PDF 报告中超监管/高置信分级警示横幅的描述。
...
**图标处理**：
- 初版使用 emoji（🛡️ ⚠️），Chromium PDF 打印 emoji 字体不可用
- 改为基本几何字符（■ ▲）+ CSS 颜色，仍不够简洁
- **最终方案：纯文本标签**，无任何图标依赖。区分方式：红底横幅标题文案 + 行底色 + 详情卡文字标注
