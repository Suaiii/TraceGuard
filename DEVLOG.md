...
### 2026-08-26 — 工作区清理与证据保留

- 已将批量输出、临时结果和缓存移出主工作区，归档至 `E:\aNB\ARCHIVE\AI竞赛_cleanup_20260826`；原始数据集已恢复在主工作区 `dataset/`，主工作区同时保留源码、配置、测试、报告源、冻结汇总、模型权重和最终交付材料。
- 按 `experiments/*/verified_results/provenance.json` 的路径约定恢复必要的逐样本原始预测文件；不匹配旧哈希的定位临时副本未放回主工作区。
- 根目录历史草稿、无关文档和过期图稿已归档；当前正在编辑的答辩 PPT、Draw.io v6 及服务占用的两个空日志保持原位，避免打断进行中的工作。
- 未修改或丢弃现有代码改动；`DEVLOG.md` 与 `docs/figures/system/system_architecture_drawio_v1.drawio` 的未提交修改继续保留。
- **作品简介已按三条主线重构**：将“多模态协同研判、真实网络传播鲁棒性与退化感知、超监管高危内容识别与分级处置”提升为第一层叙事，原有检测、热力图、定位、风险和多入口能力调整为支撑模块；明确“多模态”指视觉伪造取证与图文语义理解在系统内部协同，不声称支持音频、视频联合输入。`reports/TraceGuard_作品简介.md` 及 `作品/TraceGuard_作品简介.docx/.pdf` 已同步重建，Word 导出 PDF 为 1 页并完成逐页视觉核查。
...
**当前统一作品名称（2026-08-26 用户确认）**：`TraceGuard:面向社交媒体网络传播的 可解释 AIGC 图像取证平台`。设计报告、测试报告、作品简介、原创性声明、README、源程序包和可执行程序包均使用这一标题；不得再混用“面向跨域 AIGC 图像的可解释伪造检测与篡改取证平台”等旧标题。
...
- **2026-08-26 测试报告 v1.1 已生成**：新增 `reports/TraceGuard_测试报告.md`、`作品/TraceGuard_测试报告.docx` 和 `作品/TraceGuard_测试报告.pdf`。报告基于 `experiments/*/verified_results/` 冻结证据，补充了跨域、传播扰动、定位、风险、平台烟测和复现索引，并明确区分历史冻结基线与当前复测结果。使用当前 `E:\aNB\envs\traceguard\python.exe -m pytest tests -q` 实测收集 216 项，213 通过、3 失败：2 项 `explanation.batch --help` 子进程超过 15 秒，1 项提交包装测试因 `output/doc` 目录不存在；不得写成 216/216 通过。DOCX 经 Word 导出 PDF 后逐页渲染核查，共 9 页，无表格断裂、文字截断或页码异常。此项仅仅新增报告材料和构建脚本，未修改检测代码。
- **2026-08-26 其余提交材料已完成装配**：新增单页 `作品/TraceGuard_作品简介.docx` / `.pdf`（工作源 `reports/TraceGuard_作品简介.md`，已逐页渲染核查）；新增 `作品/TraceGuard_源程序.zip`，包含 146 个源码/配置/测试/前端/权重文件，ZIP CRC 通过且无 `__pycache__`/`.pyc`，权重 SHA-256 为 `29F85CAFFA5FCE11C7F31A2FB29C4DC44F65782D5300064BC4F73ADB153B0474`；新增 `作品/TraceGuard_可执行程序.zip`，包含静态 Windows x64 启动器 `TraceGuard.exe`、运行目录和权重，`TraceGuard.exe --help` 在指定 TraceGuard Python 环境下退出码为 0，ZIP CRC 通过。可执行程序采用“启动器 + Python 运行目录”形式，需要目标机安装依赖，不声称为免 Python 的单文件模型程序。

- **答辩 PPT 任务边界已明确（2026-08-25）**：项目背景页已改为“单张 RGB 图像输入—全局真伪判断与局部可疑证据并行—融合风险处置与检测报告”，明确当前不以视频、音频或多模态输入为对象；MambaOut、MK-MMD、fake_prob 等实现细节仅仅保留在“破局思路”，并用三个问题自然衔接下一页技术痛点。
...
- 朱羿帅 → **#16**：能力前置改写摘要/Ch1/Ch4 并替换主稿；图 2-2 真实检测证据已收口；维护 DEVLOG 与分工；超监管 3.3.4 已并入；答辩 PPT 已补齐“图像对象—全局/局部任务—融合处置”的背景衔接；已完成测试报告 v1.1 的 Markdown/DOCX/PDF 封装与逐页核查；07-20 封版匿名化复核。
...
### 2026-07-30 — feature/super-oversight-classifier-v2：分类器扩展、作品报告与 PDF 报告集成

**分支**: `feature/super-oversight-classifier-v2`（基于 main，10 commits 超前）

今日围绕超监管分类器做了三块工作：分类器 Prompt 扩展、作品报告 LaTeX 完善、系统 PDF 报告集成。

---

#### 1. 新增灾难类别

超监管领域从 4 类扩展为 5 类，新增 `disasters`（火灾/洪水/地震/通用灾难）含 4 条模板，覆盖大火、洪水海啸、地震废墟、通用灾难救援场景。全量配置现为 **10 类别 × 27 模板**（5 超监管 17 模板 + 5 普通 10 模板）。

---

#### 2. 作品报告 LaTeX 完善

在 `docs/restructure/latex_ch1_overview.tex` 中新增/更新五处：

| 位置 | 内容 |
|------|------|
| **第二章新增 2.6 节**（`sec:content-classifier`） | 内容语义分类器设计：MobileCLIP2-S0 选型理由、十类标签、三级分级触发逻辑（超监管高危 → 高置信伪造 → 普通结果）、诚实代理原则 |
| **第四章新增 4.7 节**（`sec:innovation-prompt-ensemble`） | 多模板加权融合引擎创新：多维 Prompt 模板设计、加权融合数学公式、权重设计逻辑、"强调共识、容忍分歧"软融合策略 |
| **摘要** | 补入"集成轻量级内容语义分类器…实现分级响应" |
| **LLM 研判边界** | "系统不具备内容类别分类能力"→"系统已集成…分类器输出仅用于前端优先级排序，研判文字不自主判断内容题材" |
| **总结** | 加入内容语义分类为已实现能力，简介多模板融合引擎 |

此外第 2.7 节"检测报告生成"补充了 PDF 报告中超监管/高置信分级警示横幅的描述；参考文献列表（bibliography）同步更新。

---

#### 3. 系统 PDF 报告集成超监管信息

**文件**: `explanation/visualization/report.py`（`ReportGenerator` 类）

此前报告生成器仅凭 `label=='fake' && fake_prob>=0.9 && risk_level=='high'` 触发统一的"高置信伪造"横幅，完全无视内容分类器输出。本次改动将内容分类结果接入报告渲染管线：

**单图报告（`generate_single`）**：
- 提取 `is_super_oversight_domain` 与 `content_category` 字段
- `_high_confidence_alert()` 拆分为两套文案：
  - 超监管领域 → 红底横幅 + "超监管领域高危 · 建议立即复核" + 附带具体类别名（如"战争冲突"）
  - 非超监管 → 红底横幅 + "高置信伪造 · 建议加急复核"（维持旧文案）

**批量报告（`generate_batch`）**：
- 新增 `super_oversight_count` 统计，指标栏展示"超监管高危 N | 高置信伪造 M"
- 逐条结果行：超监管行红底 `row-super-oversight`，普通高置信行黄底高亮 `row-high-risk`
- 高风险详情卡片：标题 badge 区分"超监管高危"与"高置信伪造"
- 新增 `_SO_CATEGORY_LABELS` 映射（warfare→战争冲突 等 5 类）
...
```
97959d3 fix: 移除报告所有图标 — 仅保留纯文本标签区分超监管/高置信
e247746 fix: 报告图标改用基本几何字符避免PDF emoji字体缺失
8c3ebc4 docs: 作品报告2.7节补充PDF报告超监管分级警示说明
e54e353 feat: PDF 报告加入超监管领域信息 — 单图/批量分级警示与指标统计
e7d5cbb docs: 完善作品报告——摘要/总结/LLM边界三处加入超监管分类器信息
2c94673 docs: 作品报告新增内容分类器设计要点与多模板融合创新点
16a7b85 feat: 超监管分类器新增灾难类别 — 火灾/洪水/地震/通用灾难
```
...
**改动**: `explanation/content_classifier/classifier.py` 重构（+263/-68）

---

#### 📌 问题背景：v1 的语义脆弱性

v1 分类器已集成 MobileCLIP2-S0，但 Prompt 设计上存在「单点脆弱」问题——每个类别只配了一条长描述句，例如：

> `"warfare, military conflict, armed combat, soldiers in battle, explosions on battlefield"`

这带来的隐患是：**零样本分类对措辞高度敏感**。同含义、不同措辞的 Prompt 可能产生差异化的相似度分数。若这条唯一的 Prompt 对某张图的描述能力不足（如描述偏"战场全景"而实际图片是"巷战近景"），该图就会被分到错误的类别。简单说，**用一个句子代表一个类别，就像用一张照片代表一个人的全部面貌——视角太窄了**。

#### 🎯 核心思路：从"单句投票"到"多视角共识"

v2 的核心思想直截了当：与其费心选一条"最优"Prompt，不如让每个类别配备 **多条覆盖不同视觉语境的 Prompt 模板**，在文本编码阶段就融合为一个高鲁棒性的「超特征向量」。

具体来说，每个类别定义 **2–4 条 Prompt**，从不同维度描述同一类内容：

| 视觉维度 | 说明 | weapons 类别示例 |
|----------|------|-----------------|
| **特写视图** | 近距离、细节丰富的描述 | `"a close-up photograph of firearms, handguns, rifles, or assault weapons"` |
| **环境全景** | 场景级、包含周边语境的描述 | `"military armaments, heavy weapons, missiles, and explosives"` |
| **通用叙事** | 不依赖特定拍摄手法的泛化描述 | `"a picture showing dangerous weapons, guns, or ammunition"` |

类似地，warfare 类别额外补充了**新闻纪实**维度（`"news coverage of military strikes, war zones..."`）；terrorism 类别补了**动作事件**维度（`"imagery of hostage situations, terror attacks..."`）。最终 **9 类别 × 23 条模板**，不再是 v1 的 9 类别 × 1 条。

#### 💡 算法实现：加权融合 → 超特征向量

融合逻辑在服务启动时**一次性完成**，不影响推理速度：

```
对于每个类别（如 weapons）：
  ① 将 3 条 Prompt 分别送入 CLIP 文本编码器 → 3 个 512 维向量
  ② 每个向量 L2 归一化（消除文本长度差异带来的模长偏差）
  ③ 按模板权重加权求和：
       超特征向量 = v₁ × 1.2 + v₂ × 1.0 + v₃ × 1.0
  ④ 对求和结果再次 L2 归一化 → 得到代表"weapons"的单一 512 维向量

全部 9 类 → 9 个超特征向量 → 堆叠为 [9, 512] 的 text_features 矩阵
```

**权重设计逻辑**：超监管类别中，核心判别模板（如特写枪械）给 1.2× 强化区分信号，辅助模板 0.8–1.0×；普通类别统一 1.0×。权重硬编码在 `PROMPT_CONFIG` 字典中，后续调权无需改动类逻辑。

#### ✅ 关键优势

| 优势 | 效果 |
|------|------|
| **推理零额外开销** | 融合在 `__init__` 做完，`classify()` 仍是单次图像编码 + 单次矩阵乘法，~15ms 不变 |
| **下游零改动** | 返回 dict 的 key 完全保持兼容，pipeline / API / 前端一行不改 |
| **Prompt 鲁棒性** | 多视角融合 → 单条措辞偏差被稀释 → top-1 类别更稳定 |
| **可配置** | PROMPT_CONFIG 独立于类逻辑，增删模板/调权重只需改字典 |
| **类别标识语义化** | `"warfare"` 代替 `"warfare, military conflict, ..."`，日志和 LLM prompt 中更干净 |

#### 📊 与 v1 对比

| 维度 | v1 | v2 |
|------|-----|-----|
| 每类别 Prompt 数 | 1 条 | 2–4 条（共 23 条） |
| 文本向量构造 | 单句编码 → 直接堆叠 | 多句加权融合 → L2 → 堆叠 |
| Prompt 鲁棒性 | 对措辞高度敏感 | 多视角共识，抗措辞扰动 |
| 权重可调 | 不支持 | 逐模板独立权重 |
| 推理开销 | ~15ms GPU | 相同 |

#### 🔮 后续方向

当前权重为经验手工设定（核心模板 1.2×、辅助 0.8–1.0×）。后续可在小规模标注集上用网格搜索或贝叶斯优化自动寻优；更进一步的方案是用可学习融合层（轻量 MLP 或注意力池化）替代手工加权，在小规模标注数据上端到端训练融合权重，同时冻结 CLIP 骨干以保持零样本泛化能力。
...
- **定名（历史记录，现已被 2026-08-26 用户确认标题取代）**："传播链末端"全文弃用（"末端"无法精确定义、非共识词）。当前统一标题为 **TraceGuard:面向社交媒体网络传播的 可解释 AIGC 图像取证平台**。正文精确概念在 1.1 首次出现处定义**"传播后裸图"**后沿用。背景图两处用词同步改（`传播后「裸图」`/`第三道防线：传播后第三方审核`），已重导出 png/pdf/svg。
...
### 2026-08-25 - 答辩 PPT 项目背景与任务边界修订

- 输出 `作品ppt_任务聚焦衔接优化.pptx`，保持原有 7 页结构和模板风格，仅仅重构第 3 页。
- 删除原页图像/视频混合案例及背景中的具体技术细节，明确 TraceGuard 聚焦社交媒体传播末端的单张 RGB 图像。
- 将作品任务表述为“全局真伪判断 + 局部可疑证据 + 融合风险处置”，并以跨域识别、传播证据衰减、可信取证三个问题衔接技术痛点。
- 已完成 7 页逐页渲染核查，模板保真检查 0 issue；未修改检测代码、实验数字或项目能力边界。
