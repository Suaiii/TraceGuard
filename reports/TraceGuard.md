...
针对可绕过生成端对齐并进入假新闻传播的超监管高危内容\[9\]，检测器在未见域的 9 个生成器上取得 98.84% 总体 Fake Recall，真图假阳率为 1.4%。八生成器盲测 Real Recall 为 99.80%。系统已在 RTX 4060 Laptop GPU 完成真实 Web 闭环，短时并发烟测中位延迟为 0.520 秒。
...
生成式人工智能降低了图像伪造门槛，带来假新闻、舆情误导和电子证据污染。生成端对齐也可能被战争、恐怖主义、枪械等“超监管”内容绕过\[9\]；这些高危图像还会经历平台压缩、缩放和截图转存，进一步削弱伪造证据。因此，传播链末端需要能给出依据、风险和复核记录，并在证据不足时转人工的第三方审核平台。

1\.2 相关工作与现有不足

现有方法分别从统计或频域痕迹\[1\]、预训练表征\[2\]和 Grad\-CAM 归因\[7\]开展检测，但新生成器与重编码会造成域偏移，归因结果也常未进入审核流程。GenImage\[3\]等数据以常规内容为主，针对超监管内容和真实平台传播链的同图成对评价仍不足\[9\]。TraceGuard 的切入点是把跨域检测、传播评测、并行解释证据和人工复核组织为同一闭环。

1\.3 作品定位与应用场景

作品以社交媒体高危假新闻配图筛查为核心，并覆盖平台审核、新闻核验和电子证据初筛。检测器在未见该内容域的条件下，对 9 个超监管生成器的总体 Fake Recall 为 98.84%，真图假阳率为 1.4%（表 3\.6）。平台可据风险等级分流；新闻核验可导出证据报告；取证初筛则保留 `label` 与 `tamper_type` 的来源和分歧，供人工鉴定。

1\.4 特色描述与应用前景

作品具有三项特色：一是零样本超监管识别与真图低误伤；二是全局判断、局部证据和融合结论分层输出，冲突即转人工；三是 Web、API、CLI 与批量入口复用同一管线，可在单张消费级 GPU 上运行并导出报告。传播链同图成对评测和确定性扰动进一步标定系统边界。系统可用于平台风险分流、新闻核验和证据初筛；超监管概念与数据来自公开文献\[9\]，本作品仅评价自研检测器，不涉及高危内容生成或训练。

1\.5 团队分工与阶段计划

成员 A 负责集成，成员 B 负责检测，成员 C 负责解释；7 月封版。
...
BigGAN Fake Recall 为 94.90%，VQDM 和 ADM 仅 13.20%、37.00%；Real Recall 均为 99.80%。平均 Accuracy 79.68% 不代表所有生成器均稳定。“提升 17%+”缺消融原表，暂不作为结论。
...
超监管概念与数据来自公开文献\[9\]。本作品仅以固定权重零样本评价 9 个生成器共 2250 fake 和 500 real，不生成、训练或优化高危内容。
...
九个生成器 Fake Recall 为 94.0%--100.0%，总体 98.84%，真图假阳率 1.4%。因生成器构成不同且无 VQDM、ADM 难例，仅支持“可跨内容域迁移”的方向性结论。
...
JPEG75/50 保持率仅 18.2%/8.6%，表明“零样本识别强、抗压缩弱”。结果仅刻画自研检测器边界，不构成生产保证。
...
| 验收项 | 环境或输入 | 结果 | 解释边界 |
|---|---|---|---|
| 自动化回归 | GPU 运行环境 | 190/190 通过 | 验证代码行为，不替代模型泛化实验 |
...
未见域的 9 个超监管生成器 Fake Recall 为 94.0%--100.0%，总体 98.84%，真图假阳率 1.4%（表 3\.6）；通用盲测 Real Recall 为 99.80%。因生成器构成不同且缺 VQDM、ADM 难例，仅主张识别能力可跨内容域迁移。
...
5 核高斯 MK\-MMD 无需目标域标签即可对齐多尺度分布。其独立收益仍待消融原表，不提前写为已验证提升。

4\.6 可解释取证与报告化闭环

检测分数、热力图、可疑区域、解释和报告在同一流程内生成，使结果可复核、沟通和归档。
...
TraceGuard 已实现跨域检测、Grad\-CAM、局部定位、五维风险融合以及 Web/API/CLI/报告闭环。超监管零样本 Fake Recall 为 98.84%，真图假阳率为 1.4%；Facebook 传播使 Fake Recall 从 59.55% 降至 21.675%，确定性实验进一步确认 JPEG 是主要证据破坏面。

系统坚持“全局判断、局部证据、融合结论”三层合同，证据冲突或退化即转人工，不静默改判。当前定位精度、单来源阈值和缺失的跨域消融限制了结论外推；后续工作将补齐独立来源评价与消融证据。
