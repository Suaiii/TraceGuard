...
测试覆盖八类生成器、三类社交平台传播条件与真实浏览器闭环，使用 Accuracy、Macro F1、ROC AUC、Recall、成对概率变化和性能保持率评价检测能力，并结合稳定、传播退化和证据冲突案例说明系统如何支撑人工复核。当前结果同时揭示了 Facebook 传播条件下的显著性能退化；一组 AIGC 派生定位边界评价和分层留出风险校准已完成，但仍需更多独立来源复核。
...
系统最终保持“全局判断、局部证据、融合结论”三层输出。全局 `label` 与 `fake_prob` 仅由 Detector 产生；热力图与局部定位作为并行证据分支；风险模块依据多维证据给出审核优先级和人工复核建议。当前来源的定位边界评价与留出风险校准已经写入报告，但其低像素重合、clean 误报和单一传播来源限制了外推范围；消融原始表和更多来源复核仍不能写成已验证结论。
