...
**成员 A（系统集成与实验分析）**：负责总体架构设计、多证据融合管线集成、Web/API/CLI/批量四入口统一接口实现、全局判定与局部证据的三层输出合同设计及代码级一致性保障；主导社交媒体传播链成对评测（8000 组 Original→Facebook/WeChat/Weibo，32000 次预测）、确定性派生扰动实验（JPEG/Resize/Screenshot，40000 次预测）与超监管高危内容零样本边界评测（纯公开子集，9 生成器 × 250 张）；完成平台短时并发烟测（RTX 4060 Laptop GPU，中位延迟 0.520 s）、报告能力前置叙事重构（摘要/第一章/第四章改写）、三张系统结构图重做（PPT 全原创素材 + COM 渲染导出）、Word 工作稿生成（41 个原生 OMML 公式、0 残留 LaTeX）、全量测试维护（191/191 通过）、提交包运行白名单装配与匿名化红线审计；维护 DEVLOG 与团队分工协调。

**成员 B（跨域 AIGC 检测与模型训练）**：负责跨域检测模块的核心算法设计与实验验证——选取 MambaOut-Small 门控卷积骨干并去除全局池化以保留空间伪影特征（2304 维展平 → 256 维瓶颈层），引入五核高斯 MK-MMD 无监督域自适应对齐源域与目标域特征分布，设计差分学习率、Warmup、余弦退火与梯度裁剪的联合训练策略；完成 GenImage 八生成器平衡盲测（16000 张，输出 eval_results.csv），实施 MK-MMD 消融实验（两次独立训练，seed=42，八生成器平均 Fake Recall 从 49.59% 提升至 59.55%，绝对 +9.96 pp，相对 +20.08%）；编写 REPRODUCIBILITY.md（数据划分、训练配置、复现命令与脚本口径说明）。

**成员 C（可解释取证与风险复核）**：负责可解释性分析与篡改定位模块的定量评价体系建设——搭建合成篡改数据集生成管线与定位评价框架（IoU/Dice/Pixel F1/Detection Rate，含 CASIA v1 与 AIGC 合成两套评价），实现风险阈值分层校准与留出评估（策略 A vs B 对比，留出集复核 F1=0.9877，高风险 F1=1.0）；从 32000 条成对预测中固定稳定/衰减/冲突三类典型案例，完成传播链案例自动分类（3 stable / 1 degradation / 2 conflict_degraded / 3 conflict pairs），生成三类案例证据图（红框 bbox 叠加、中文化数据标注、行为叙事框、学术高级感卡片风格、PIL 纵向拼接总图），补齐 6 条实验线的 README + provenance.json 交付合规审计。

**阶段计划**：2026 年 7 月 13 日完成跨域盲测、传播成对实验与典型案例固定；7 月 14 日完成扰动实验全量运行与超监管叙事方案落地；7 月 15 日完成报告能力前置叙事转向与系统结构图重做；7 月 17 日完成超监管数据来源纠偏（纯公开子集）、三类案例图全套产出与交付合规补齐；7 月 18 日完成消融实验受控变量声明修补与案例总图学术重构；7 月 19 日冻结技术内容；7 月 20 日处理封版阻塞问题，完成匿名化终审。
