...
| 验收项 | 环境或输入 | 结果 | 解释边界 |
|---|---|---|---|
| 自动化回归 | Python 3.10.20，GPU 运行环境 | 172/172 通过 | 验证代码行为，不替代模型泛化实验 |
...
上述结果仅证明 Web/API/CLI 闭环和固定样例的短时并发可用，不支持长时稳定性、容量、多输入或阈值合理性主张。传播实验也仅为固定权重、三类平台的一次运行，分类集 fake:real 为 9:1；定位评价、风险校准和跨域消融仍待补证据。
