...
TraceGuard 是面向社交媒体网络传播的可解释 AIGC 图像取证平台。系统集成跨域检测、Grad\-CAM、局部定位、五维风险融合与报告导出，并以“全局判断、局部证据、融合结论”三层合同保证 Web、API、CLI 结论一致；证据冲突或退化时转人工，不静默改写全局标签。
...
测试围绕两个核心问题展开：第一，检测器能否在未见过的生成器上保持判别力（跨域泛化）；第二，图像经社交媒体平台传播后，检测证据是否仍可靠（传播鲁棒性）。前者验证系统的核心能力边界，后者度量系统在真实部署链路上的退化风险——两者合起来构成"能力 + 边界"的完整评估框架。

围绕上述两问，测试覆盖五个维度：跨生成器平衡盲测（3.3.1）标定泛化基线；社交媒体成对传播评测与平台分类集（3.3.2）度量真实链路退化；确定性扰动拆解（3.3.3）定位退化主因；超监管高危内容零样本评测（3.3.4）检验跨内容域迁移能力；可解释案例、定位边界与平台闭环验收（3.4--3.5）验证证据输出与工程完整性。全部预测由同一份固定权重和统一 `Detector` 产生（总计约 10 万次预测、0 运行时失败），逐样本结果与运行环境信息均已冻结于 `verified_results/`，确保可复现审计。
...
MK-MMD 域自适应使检测器习得域不变特征表示（见 4.5），可在未参与训练的生成器上保持判别力。在 9 个完全未见超监管生成器的纯公开子集上（2250 张 fake），零样本总体 Fake Recall 为 98.00%（92.4%--100.0%，见表 3.6）；通用八生成器盲测 Real Recall 为 99.80%，确保真图低误伤。因公开子集生成器构成比 GenImage 容易（无 VQDM、ADM 难例），仅主张识别能力可跨内容域迁移，不与 GenImage 逐项对比。

4\.2 三层输出合同与可信路由：证据退化即转人工

系统在架构层面强制实施三层输出合同：全局 `label` 与 `fake_prob` 仅由 Detector 产生（唯一权威来源），Grad-CAM 热力图与局部定位作为并行证据分支独立运行，风险融合模块汇总各方信息给出分级和处置建议但不改写全局标签。全局判定与局部定位之间构建四象限证据分歧保留机制——当全局判真但局部检出异常区域（`local_tamper`）或全局判伪但无可疑热点（`full_aigc`）时，两类证据并行输出、保留分歧，触发人工复核而非由任一路径静默覆盖另一路。留出集复核 F1 为 0.9877、高风险 F1 为 1.0，表明分流机制在高风险样本上做到零遗漏的前提下有效控制人工复核总量。阈值仅适用于当前单一来源，未替换生产配置，不外推至其他数据域。

4\.3 平台的自我诊断能力：传播链末端证据退化的系统度量

传统跨截面评测无法区分内容差异与平台处理各自对检测性能的影响。本作品采用同图成对协议——同一 `sample_id` 的 Original 与平台传播版本成对比较（8000 组 × 4 条件），消除内容差异作为混淆变量，实现对平台处理效应的因果度量。结果显示 Facebook 传播使 Fake Recall 从 59.55% 降至 21.675%、平均伪造概率下降 0.3162。

在观察性成对评测之外，进一步引入确定性扰动作为可控因果拆解工具：JPEG-75/50 重编码、Resize-0.5x 缩放和截图模拟四种干预独立施加于同一批图像。JPEG-75/50 使 Fake Recall 保持率仅 16.65%/3.93%，与 Facebook 退化方向一致且量级更大，双链互证将主破坏面锁定为 JPEG 有损压缩。Resize-0.5x 表面使保持率升至 137.94%，但真图假阳检查（1000 张真图，假阳率 1.50%→17.50%，160 例 real→fake 单向翻转）证伪了这一"提升"——实为重采样偏置，非鲁棒性收益。系统据此建立自我诊断闭环：当证据退化超阈值或真图假阳异常升高时，自动识别结论不可靠并转人工，不以低证据状态做出高置信判定。
...
引入五核高斯 MK-MMD 将源域与目标域的 256 维瓶颈特征映射到再生核希尔伯特空间，通过最小化经验 MMD 距离学习域不变表示。五个核带宽按几何级数分布（\(2^{i-3} \cdot \bar{d}\)），覆盖从四分之一到四倍基准带宽的尺度范围，分别捕获不同宽度的分布差异。训练采用渐进 β 调度——初期 β=0 让分类器充分学习源域判别特征，随后线性增长至 1.0 逐步引入域对齐，配合差分学习率（骨干 \(1\times10^{-5}\)、新增层 \(1\times10^{-3}\)）防止预训练知识被冲刷。消融实验显示 MK-MMD 使八生成器平均 Fake Recall 从 49.59% 提升至 59.55%（绝对 +9.96 pp，相对 +20.08%，见表 3.1b），BigGAN 提升最大 (+21.6 pp)，ADM 和 Wukong 亦显著。（消融为两次独立训练，+9.96 pp 含训练随机性成分，不可全归因于 MK-MMD。）
...
约 45.44M 参数的模型已在 RTX 4060 Laptop GPU 完成 Web 闭环，12 请求并发烟测全部成功、中位延迟 0.520 秒（见表 3.9）。Web、FastAPI、CLI 与批量分析四个入口共享同一 `ExplanationPipeline` 实例与输出合同（字段、类型、语义三统一），前端与脚本不各自重新计算真伪结论。全量自动化测试 191/191 通过，覆盖检测、热力图、定位、风险、管线、报告生成与提交包装配，确保迭代过程中合同不被静默破坏。
