...
当前可复用的 FakeTrace 工程基础已经将主运行逻辑统一到 src/faketrace\_app/，根目录入口保留 app\.py、web\_app\.py 等薄封装。TraceGuard 在此基础上聚焦图像检测、可解释展示、篡改定位和报告导出，支持 CLI、Web UI 和 FastAPI 三类入口，便于演示、批量测试和后续系统集成。
...
TraceGuard 的报告主线不展开多模态比赛叙事，而是明确聚焦 AIGC 图像证伪检测。底层 FakeTrace 工程基础中已有的其他模态代码可作为后续扩展储备，但本作品报告的系统设计、测试分析和创新性说明均围绕跨域图像检测、可解释取证和篡改定位展开。

# 第三章 作品测试与分析

3\.1 测试目标与总体方案

测试目标是验证 TraceGuard 在真实图像传播环境中的可用性、检测准确性、跨域泛化能力、扰动鲁棒性和报告化闭环能力。测试章节需要说明数据来源、验证集来源、指标定义、baseline 或对照方法、实验目的、结果解释和局限性。
...
为全面评估本方法在未知目标域上的泛化能力，以 eximage 为统一源域，在 GenImage 数据集的 8 个生成器子集上测试跨域盲测指标，并与现有 AIGC 检测方法进行对比。实验采用 Accuracy、F1、AUC 和跨域性能下降幅度作为评价指标。

表 3\.3 · 与现有方法的跨域盲检测对比

3\.4 可解释与篡改定位案例分析

案例分析应选择真实图、AIGC 图和局部篡改图各若干张，展示原图、检测结果、热力图或定位掩膜、风险等级和报告化解释。分析重点不是堆叠图片，而是说明模型证据如何支持最终判断，以及哪些场景仍需要人工复核。
...
当前框架阶段仍需补齐正式图像实验结果、可解释模块定量评价、复杂传播链路中的鲁棒性验证和平台压力测试。对于尚未完成的扩展能力，应明确其工程边界，避免把底层仓库能力写成当前作品已经完成的核心效果。
