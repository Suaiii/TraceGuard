...
| **入库证据** | **`experiments/<线>/verified_results/`** | **tracked；必须齐备 README.md + provenance.json + 报告级汇总（见 §12.1）** |
| 逐样本原始预测 | `results/<线>/` | ignored；由对应 `provenance.json` 的 SHA-256 锚定 |
| 构建产物 | `output/`（docx、figures、submission） | ignored；只放构建产物，不作为证据落脚点 |
| 其他运行输出 | `case_study/`、`tmp/`、`tmp_*` | ignored 或任务后清理 |
| 竞赛原件 | 工作区根目录的指南、模板、声明 | external-reference 或经确认后 tracked |
| 正式报告快照 | `reports/` | 仅保留明确版本和来源 |

### 12.1 verified_results 约定（唯一入库证据归宿）

`experiments/<线>/verified_results/` 是入库证据的**唯一**归宿。任何进入报告、答辩或 README 的实验数字，必须能追溯到某个 `verified_results/` 目录。实验线各自发明摆法、把证据留在 `output/` 或仓库根目录，一律视为违规。

每个 `verified_results/` 目录必须齐备三件：

1. **`README.md`**——**口径边界**。写清结果文件清单，以及每个数字**能支持什么、不能支持什么**。数据只含 fake 就必须写明不得计算 Accuracy/Macro F1/ROC AUC；反常数字必须写明其性质与需要配合解读的对照实验；构成不同的实验必须写明不可互相替代。样板见 `experiments/socialmedia/verified_results/README.md`，新写的 README 用中文并沿用其口径措辞。
2. **`provenance.json`**——**哈希锚点**。至少记录 `frozen_at`、`runtime`、权重 `sha256`、数据归档 `sha256`、逐样本 raw CSV 的 `sha256` 及其本地路径、入库汇总文件自身的 `sha256`，以及 `cross_checks`。字段缺失时如实写 `null` 或 `"待补"`，并在 `gaps` 中说明，**不得推断或编造**。
3. **报告级汇总 CSV/JSON**——小型、可进 Git、可直接支撑报告表格。

逐样本原始预测统一放本地 `results/<线>/`（gitignored），**不进 Git**，靠 `provenance.json` 的哈希锚定。大文件不进 Git，但证据链不能因此断裂：哈希是原始产物与入库汇总之间唯一的连接。

`output/` 只放构建产物（docx、figures、submission），**不再作为证据落脚点**。实验跑完后产物先落 `output/`，冻结时必须把汇总迁入对应 `verified_results/` 并补齐 README 与 provenance；未迁入的一律视为未入库，不得进入报告。

当前实验线：`socialmedia/`、`perturbation/`、`crossdomain/`、`localization/`、`risk/`、`platform/`。
...
- 凡图中出现箭头，Draw.io 的“线末端”大小必须统一设为 `0 pt`（对应样式属性 `endSize=0`）；箭头类型按语义保留，不得使用默认 `6 pt` 或其他末端大小。
