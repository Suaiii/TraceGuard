...
1. `README.md`：当前可运行入口、模块结构、接口和已验证能力。
2. `docs/project_plan.md`：正式题目、三人分工、目标和阶段安排。
3. `docs/submission_progress.md`：报告快照、当前缺口和近期行动。
4. `configs/default.yaml`：检测、热力图、定位、风险、文本与输出参数的配置真相源。
5. `explanation/api/schemas.py`：HTTP 请求和响应的数据合同。
6. `explanation/api/routes.py`：Web 静态资源、健康检查、配置、单图和批量分析接口。
7. `tests/`：行为回归准入；README 中的测试数字不得替代实际测试结果。
8. `开放式自主命题作品赛参赛指南.pdf`、`作品报告模板.docx`、`原创性说明.docx`：竞赛与交付格式依据。
9. `竞赛计划.md`：早期竞赛计划。若与仓库内 `docs/project_plan.md` 冲突，以后者和用户最新确认口径为准。
